...
///   create OpenMint SRC20 -> mint -> transfer -> market buy/sell create only (no matching).
contract Stage7LiveChainFlowNoMatchScript is Script {
    ...
    function run() external {
        ...
        // Buyer creates buy order (挂单，不成交).
        ...
        // Buyer creates sell order (挂单，不成交).
        ...
        require(sellPrice > 0, "SELL_PRICE_ZERO");

        require(_queryOrderStatus(market, buyOrderId) == 1, "BUY_NOT_OPEN");
        require(_queryOrderStatus(market, sellOrderId) == 1, "SELL_NOT_OPEN");
        ...
        require(buyerTokenAfter > 0, "BUYER_NO_TOKEN");
        ...
        console2.log("LIVE_CHAIN_FLOW_BUY_ORDER_STATUS", _queryOrderStatus(market, buyOrderId));
